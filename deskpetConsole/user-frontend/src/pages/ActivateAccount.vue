...
<template>
  <div v-if="isMobile" class="mobile-auth-page">
    <section class="mobile-status-card">
      <div class="mobile-status-icon">{{ loading ? '…' : success ? '✓' : '!' }}</div>
      <h1 class="mobile-status-title">{{ loading ? '正在验证激活链接' : success ? '账号激活成功' : '激活未完成' }}</h1>
      <p class="mobile-status-description">
      ...
      <section class="mobile-status-note">
        <div class="mobile-note-title">{{ loading ? '处理中' : success ? '接下来可以这样做' : '你可以这样处理' }}</div>
        <p class="mobile-note-text">
      ...
      <RouterLink v-if="success" to="/auth/login" class="mobile-submit-button mobile-submit-link">去登录</RouterLink>
      <button v-else-if="!loading" type="button" class="mobile-submit-button" @click="runActivation">重试激活</button>
      <p v-if="!success" class="mobile-auth-footnote">
        需要重新开始？
        <RouterLink to="/auth/register" class="mobile-inline-link">返回注册</RouterLink>
      </p>
    </section>
  </div>
