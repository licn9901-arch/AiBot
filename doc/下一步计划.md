...
| V0.4 | 前端分离 | 管理后台增强（✅ 已完成） + 用户端（✅ 已完成） |
| V0.5 | 可观测性 + 实时推送 | TimescaleDB 时序库（✅ 已完成）+ WebSocket 实时推送（✅ 已完成） |
...
### 6.4 WebSocket 实时推送（✅ 已完成）

**方案：Spring WebSocket + STOMP over SockJS**

选择理由：
- Spring 原生支持 STOMP，配置简单
- 内置 topic 订阅机制，天然支持按设备 ID 推送
- Sa-Token 可在 STOMP CONNECT 阶段通过 header 传递 token 认证
- SockJS fallback 保证浏览器兼容性

实现内容：
- WebSocket 端点 `/ws`（SockJS），STOMP broker 前缀 `/topic`
- STOMP CONNECT 认证拦截器（从 Authorization header 提取 Bearer token）
- WebSocketPushService 封装推送逻辑
- CommandService 在指令状态变更时推送 `/topic/device/{deviceId}/command-status`
- DeviceService 在设备上下线时推送 `/topic/device/{deviceId}/presence`
- user-frontend 移除轮询，通过 WebSocket 实时接收指令状态和设备在线状态
- manager 通过 WebSocket 自动刷新指令时间线，保留手动刷新按钮作为 fallback
...
| 消息推送 | WebSocket + STOMP | SSE | ✅ V0.5 已实现 |
