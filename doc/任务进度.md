...
- [x] 接入数据库（PostgreSQL/MySQL），完成表结构设计与迁移脚本。
- [x] 设备密钥加密存储（hash + 盐），补充注册与鉴权流程。
- [x] 遥测/指令持久化完善（latest + 历史可选）。
- [x] 指令超时与重试策略明确（默认不重试，可手动重试）。
- [x] 对外 API 文档（OpenAPI/Swagger）与示例请求补充。

### mqtt-gateway（Vert.x）
- [x] 上下行统计与连接日志完善（在线数、telemetry/ack 计数）。
- [x] /internal/auth 与内部回调异常处理策略（超时、失败重试或降级）。
- [x] 可观测性扩展（可选 Prometheus 指标）。

### 测试与模拟
- [x] 设备模拟脚本（MQTT 行为：telemetry/ack/重连）。
- [x] 协议测试（telemetry/ack）与核心状态机测试。
- [x] 压力冒烟：连续 100 次命令下发与回执闭环验证。
- [x] 异常场景：网关/核心重启、断网重连。

### 部署与运维
- [x] docker-compose（DB、可选 Redis）与启动脚本。
- [x] 环境变量与配置说明（端口、内部 token、超时参数）。
- [x] 日志级别与输出策略（控制台/文件落盘可选）。
