...
| reqId | 消息唯一标识（UUID），用于命令回执或设备请求响应的幂等、追踪、审计 |
...
| 设备订阅 | `pet/{deviceId}/resp` | 1 | 设备主动请求的业务响应（V0.6） |
| 设备发布 | `pet/{deviceId}/telemetry` | 0 | 遥测上报 |
| 设备发布 | `pet/{deviceId}/cmd/ack` | 1 | 指令回执 |
| 设备发布 | `pet/{deviceId}/event` | 1 | 事件上报（V0.3） |
| 设备发布 | `pet/{deviceId}/req` | 1 | 设备主动业务请求（V0.6） |

### 4.2 ACL 校验规则

网关强制校验：
- clientId == deviceId
- publish topic 只能是 `pet/{deviceId}/telemetry`、`pet/{deviceId}/cmd/ack`、`pet/{deviceId}/event`、`pet/{deviceId}/req`
- subscribe topic 只能是 `pet/{deviceId}/cmd`、`pet/{deviceId}/resp`
...
| DeviceRequestService | 处理设备主动请求，路由天气/对话/配置能力并返回响应（V0.6） |
...
| 内部 | InternalController | 网关回调与下行通道（telemetry/ack/event/request/response/presence） |
...
| POST /internal/request/{deviceId} | 设备主动请求（V0.6） |
| POST /internal/response/send | 向设备下发业务响应（V0.6） |
...
- 在线会话管理
- Topic ACL 校验
- 上行转发到 Core（telemetry/ack/event/request）
- 下行从 Core 接收并转发给设备（cmd/resp）
...
```
POST /internal/response/send
{
  "deviceId": "pet001",
  "topic": "pet/pet001/resp",
  "qos": 1,
  "payload": "{...json...}"
}
```
