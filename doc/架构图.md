...
%% Internal integration
    G1 -->|Internal HTTP<br/>/internal/telemetry<br/>/internal/ack<br/>/internal/event<br/>/internal/request<br/>/internal/online| C1
    C2 -->|Internal HTTP<br/>/ internal/command/send| G1
    C7 -->|Internal HTTP<br/>/internal/response/send| G1
...
```mermaid
flowchart LR
    D[Device] -->|"telemetry<br/>pet/{id}/telemetry"| G[mqtt-gateway]
    G -->|"POST /internal/telemetry/{id}"| C[pet-core]
    C -->|"UPSERT latest + append history"| DB[(DB/TSDB)]

    D -->|"req QoS1<br/>pet/{id}/req"| G
    G -->|"POST /internal/request/{id}"| C
    C -->|"业务处理 / 调用天气或 AI"| S[pet-core service]
    S -->|"POST /internal/response/send"| G
    G -->|"resp QoS1<br/>pet/{id}/resp"| D

    C -->|"POST /internal/command/send"| G
    G -->|"cmd QoS1<br/>pet/{id}/cmd"| D
    D -->|"cmd/ack QoS1<br/>pet/{id}/cmd/ack"| G
...
sequenceDiagram
  ...
  G-->>D: SUBACK
  D->>G: SUBSCRIBE pet/{deviceId}/resp
  G-->>D: SUBACK
...
sequenceDiagram
  ...
  participant G as mqtt-gateway (Vert.x)
  participant C as pet-core (Spring)
  participant AI as pet-ai / Weather Provider

  D->>G: PUBLISH pet/{id}/req (QoS1, reqId, type=getWeather)
  G->>C: POST /internal/request/{id}
  C->>AI: 查询天气 / 组装业务结果
  AI-->>C: 天气结果
  C->>G: POST /internal/response/send(reqId,...)
  G->>D: PUBLISH pet/{id}/resp (QoS1, reqId, ok/payload)
  D->>D: 按 reqId 匹配响应并播报/显示
```
...
sequenceDiagram
  ...
  UI->>C: STOMP CONNECT (Bearer Token)
  C->>C: Sa-Token 验证
  C-->>UI: CONNECTED

  UI->>C: SUBSCRIBE /topic/device/{id}/command-status
  UI->>C: SUBSCRIBE /topic/device/{id}/presence

  Note over C,WS: 指令状态变更时
  C->>WS: pushCommandStatus(deviceId, response)
  WS-->>UI: MESSAGE /topic/device/{id}/command-status

  Note over C,WS: 设备上下线时
  C->>WS: pushPresence(deviceId, online)
  WS-->>UI: MESSAGE /topic/device/{id}/presence
```
