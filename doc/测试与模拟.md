...
启动设备模拟器（telemetry/ack/重连；如后续支持可扩展 req/resp）：
...
如已实现设备主动请求接口，可额外验证天气查询链路：

1. 设备发布 `pet/pet-001/req`
2. mqtt-gateway 转发到 `pet-core`
3. `pet-core` 处理后下发 `pet/pet-001/resp`
4. 设备按 `reqId` 收到并消费响应
