...
### 2.4 WebSocket 配置（V0.5 起）

pet-core 内置 WebSocket + STOMP 支持，用于实时推送指令状态和设备上下线事件。

| 配置项 | 说明 |
| --- | --- |
| WebSocket 端点 | `/ws`（SockJS fallback） |
| STOMP broker 前缀 | `/topic` |
| 认证方式 | STOMP CONNECT 帧中携带 `Authorization: Bearer {token}` |

**注意事项**：
- `/ws/**` 路径已从 Sa-Token HTTP 拦截器中排除，认证在 STOMP CONNECT 阶段完成
- 前端开发时需在 Vite proxy 中配置 `/ws` 代理：
  ```javascript
  '/ws': { target: 'http://localhost:8080', changeOrigin: true, ws: true }
  ```
- 生产环境使用 Nginx 反向代理时，需配置 WebSocket 升级：
  ```nginx
  location /ws {
...
| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `callback.timeoutMs` | `2000` | telemetry/ack/event/request/presence 回调超时 |
...
| `deskpet_gateway_request_total` | counter | 设备主动请求次数（V0.6） |
...
```bash
# SockJS info 端点（验证 WebSocket 服务可用）
